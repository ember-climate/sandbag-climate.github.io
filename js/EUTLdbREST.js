...
function getInstallationsForCountryAndSectorAllocations(serverURL, countryNames, sectorNames, isSandbagSector, powerFlag, periodNames,  onLoadEnd){
    var query = {
	    "statements" : [ ]
	};
    ...
    if(isSandbagSector){
        ...
        if(powerFlag == "Include Power installations"){
            ...
            statementSt = "MATCH (c:COUNTRY)<-[:INSTALLATION_COUNTRY|AIRCRAFT_OPERATOR_COUNTRY]-(node)-[:INSTALLATION_SECTOR|AIRCRAFT_OPERATOR_SECTOR]->(s:SECTOR)<-[:AGGREGATES_SECTOR]-(ss:SANDBAG_SECTOR), (node)-[aa:ALLOWANCES_IN_ALLOCATION]->(p:PERIOD) " +  
                "WHERE c.name IN " + countryNames + " AND ss.name IN " + sectorNames + " AND (node:INSTALLATION OR node:AIRCRAFT_OPERATOR) AND node.latitude <> '0' AND node.latitude <> '' AND node.longitude <> '0' AND node.longitude <> '' AND p.name IN " + periodNames + 
					   " RETURN node.id, node.name, node.latitude, node.longitude, ss.name, node.city, node.address, sum(aa.value)";
